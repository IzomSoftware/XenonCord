...
jlong JNICALL Java_net_md_15_bungee_jni_cipher_NativeCipherImpl_init(
    JNIEnv *env, jobject obj, jboolean forEncryption, jbyteArray key) {
  const JNIEnv toEnv = *env;
  const jsize keyLen = toEnv->GetArrayLength(env, key);

  crypto_context *crypto = malloc(sizeof(crypto_context) + keyLen);
  if (!crypto) {
  ...
  toEnv->GetByteArrayRegion(env, key, 0, keyLen, (jbyte *)&crypto->key);
...
void Java_net_md_15_bungee_jni_cipher_NativeCipherImpl_free(JNIEnv *env,
                                                            jobject obj,
                                                            jlong ctx) {
...
void Java_net_md_15_bungee_jni_cipher_NativeCipherImpl_cipher(
    JNIEnv *env, jobject obj, jlong ctx, jlong in, jlong out, jint length) {
  crypto_context *crypto = (crypto_context *)ctx;

  mbedtls_aes_crypt_cfb8(&crypto->cipher, crypto->mode, length, crypto->key,
                         (byte *)in, (byte *)out);
}
